...
import aiofiles
import aiohttp  # need version 4.0.0.a to handle disconnects properly!
...
class Server:
    ...
    async def _init_app(self):
        ...
        app.router.add_get("/images", self.images)
    ...
    async def images(self, request):
        ...
        Requests always come in in form /images?url=image_url
        First, there's some clean-up done on the images folder, then
        Image_url is downloaded if hasn't been already, and then returned
        This ensures all images are served from the same domain (better browser compatibility)
        """
        params = request.rel_url.query
        # get images folder path
        dp = os.path.join('.', 'images')
        if not os.path.exists(dp):
            os.mkdir(dp)

        # Do some clean-up of the images folder
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not os.path.exists(fp):
                async with session.get(url) as resp:
                    if resp.status == 200:
                        fp = os.path.join('.', 'images', url_hash + '.jpeg')
    ...
    async def _send_sonos(self, request, ws_identifier, sonos_name):
        ...
        try:
            ...
            img_src = f"{self.protocol}://{HOST_IP_ADDRESS}:{self.port}/images?url={img_src}"
            # If status is PLAYING, display artwork, title, etc...
            if transport == "PLAYING":
                html = f"""
                <div><img class="artwork" src="{img_src}"></img></div>
                <div class="title">{title}</div>
                <div class="artist">{artist} - {album}</div>
                """
            # ...else say nothing is playing.
            else:
                html = '<div class="artist">Nothing playing right now...</div>'
            logger.info(f'Sending to client {ws_identifier}: {html}')
            # send HTML
